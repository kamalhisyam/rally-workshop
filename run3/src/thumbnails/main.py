...
from tensorflow.feature_column import numeric_column as numeric_col
...
from train_model import train_channel, pred_channel_output
...
feature_cols = [numeric_col(ft, dtype=tf.float64) for ft in df_train.columns]
...
r_pred_arr = pred_channel_output(df_test, r_linear_est, 'R')
g_pred_arr = pred_channel_output(df_test, r_linear_est, 'G')
b_pred_arr = pred_channel_output(df_test, r_linear_est, 'B')
...
convert_1D_array_to_img(img_arr=img_arr, size=test_img.size,
